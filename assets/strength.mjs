import { L, KEYS, QA, CATALOG_VERSION, QUESTION_MAP } from './sf-data.mjs';
import { evaluate, answerRecord, DEFAULT_WEIGHTS } from './sf-engine.mjs';
import { drainOutbox } from './sf-sync.mjs';
const $ = s => document.querySelector(s);
const STORE = 'sf2.deluxe.'+CATALOG_VERSION, CONFIG_STORE='sf2.config.'+CATALOG_VERSION, PHASES = ['A','B','C','D'];
...
async function begin() {
  ...
  catch { config=read(CONFIG_STORE); }
  try {
    if(!config){status('首次开始需要连接服务。连接成功后可保存进度并离线续答。',true);return;}
    if(config.catalogVersion!==CATALOG_VERSION) { status('题库已更新，请刷新页面后开始。',true); return; }
    try{localStorage.setItem(CONFIG_STORE,JSON.stringify(config));}catch{}
...
function choose(index) {
  if(conflict)return;
  if(!evaluate(active().answers,active().weights).next?.options[index])return;
  selected=index;
...
function renderQuestion(restored=null) {
  ...
  const progress=pi===0?state.answered/QA.length*40:pi===1?40+s.answers.filter(a=>a.phase==='B').length/Math.max(1,state.bTotal)*40:pi===2?80+s.answers.filter(a=>a.phase==='C').length/Math.max(1,state.depthTotal)*17:97;
  $('#progress').value=Math.min(97,progress);$('#progress').setAttribute('aria-valuetext',`第 ${pi+1} 阶段，共 4 个阶段`);
  $('#q-kicker').textContent=state.phase==='A'?q.scene:state.phase==='C'?'换一个经历，再确认一下':state.phase==='D'?'揭晓之前，听听你的直觉':'想想你最近的真实经历';
  $('#q-stem').textContent=q.stem;
  $('#q-help').textContent=state.phase==='A'?'两种反应都可以。凭第一反应，选更接近你平时会做的那一种。':state.phase==='D'?'选你认为最可能的结果。这一题不影响计分。':'只回想实际发生过的经历，不需要现在尝试。没有可比经历时，可以选“暂不判断”。';
  $('#keyboard-help').textContent=state.phase==='A'?'按 1 / 2 或 A / B 选择，Enter 继续':state.phase==='D'?'选一个最接近你直觉的方向':'按 1–5 或 A–E 选择，Enter 继续';
  $('#options').className='options'+(state.phase==='D'?' prediction':'');
  $('#options').innerHTML=q.options.map((o,i)=>`<button class="option${o.scored===false?' unscored':''}" aria-pressed="false" data-choice="${i}"><span class="option-key" aria-hidden="true">${i<5?'ABCDE'[i]:''}</span><span>${esc(o.text)}${state.phase==='D'?`<small>${esc(L[o.leaf].ex)}</small>`:''}</span><span class="check" aria-hidden="true">✓</span></button>`).join('');
...
function renderResult() {
  ...
  if(!win){
    $('#report').innerHTML='<article class="result-hero"><span class="eyebrow">这次先不下结论</span><h1 tabindex="-1">还需要一些真实经历</h1><p>情境选择已经完成，但后续没有足够的可判断经历，暂时无法给出一个有证据支持的强项。这不代表你的能力弱。</p><p class="notice">你的记录已经保留。之后有更具体的经历时，可以重新开始。</p></article><div class="report-actions"><button class="button primary" id="again">重新发现</button><button class="button secondary" id="print">打印 / PDF</button></div>';
    $('#again').onclick=()=>$('#restart-dialog').showModal();$('#print').onclick=()=>window.print();show('result');$('#report h1').focus({preventScroll:true});resumeLabel();return;
  }
  const evidence=s.answers.filter(a=>a.leaf===win.key && (a.phase==='B'||a.phase==='C') && !a.skipped);
...
// Content revisions must never reinterpret old answer indices as new answers.
const old = read('sf2.deluxe.v1') || read('sf2.v1');
if (old) {
  $('#legacy').hidden = false;
  $('#legacy').innerHTML = '题库已全面更新，旧记录仍然保留。<a href="/v2-previous.html">查看或继续旧题库进度 →</a>';
}
resumeLabel();
if (active()) void flush();
